...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    def mixed_loss(self, x, out_dict):
        ...
        # G6 Save samples for presentation
        g6_visual = False
        if g6_visual:
            test_identifier = "v1"
            real_sample = x[0:1]
            x_num = real_sample[:, :self.num_numerical_features]
            x_cat = real_sample[:, self.num_numerical_features:]
            t, _ = self.sample_time(1, real_sample.device, 'uniform')

            # Save real sample
            np.save(f'samples/g6_real_sample_{test_identifier}.npy', real_sample.detach().cpu().numpy())
            ...
            if x_num.shape[1] > 0:
                ...
                np.save(f'samples/g6_noised_sample_{test_identifier}.npy', x_num_t.detach().cpu().numpy())
            ...
            np.save(f'samples/g6_synthetic_sample_{test_identifier}.npy', model_out.detach().cpu().numpy())

            exit()  # stop after saving
